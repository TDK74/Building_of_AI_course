...
def main(X_train, X_test, y_train, y_test):
    ...
    for i, test_item in enumerate(X_test):
        ...
        for closest in nearest:
            lines.append(np.stack((test_item, X_train[closest])))

        y_predict[i] = np.round(np.mean(y_train[nearest]))          # this just classifies everything as 0
